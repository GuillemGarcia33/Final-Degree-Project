...
bomo_pattern = re.compile(r"Bomo_Chr\d+")
chromosome_mapping = {}
chr_counter = 1
        ...
        with open(file, "r") as f:
            for line in f:
                ...
                if len(parts) >= 3 and '@TR_' not in parts[0] and not parts[0].startswith("#"):
                    ...
                    # Filter: only process Bomo_ChrX chromosomes
                    if bomo_pattern.fullmatch(original_chrom):
                        # Assign a new name if not already done
                        if original_chrom not in chromosome_mapping:
                            chromosome_mapping[original_chrom] = f"chr{chr_counter}"
                            chr_counter += 1
